...
export function LedgerReportPage() {
  ...
  const exportCsv = () => {
    if (mode === 'group' && view === 'summary') {
      downloadCsv(`group-${title}.csv`, ['S.No.','Ledger','Opening','Debit','Credit','Closing'], filteredGroupSummary.map((row, index) => [index + 1, row.account.name, row.opening, row.debit, row.credit, row.closing]))
      return
    }
    downloadCsv(`${mode}-${title}.csv`, ['S.No.','Date','Ledger','Voucher Type','Voucher No.','Particulars','Narration','Debit','Credit','Running Balance'], filteredStatementRows.map((row, index) => [index + 1, row.date_bs, row.account?.name || '', row.voucher_type, row.voucher_no, row.particulars, row.narration, row.debit, row.credit, row.displayBalance]))
  }
