...
function TransactionRegisterTable({ report, onSelect }: { report: ReturnType<typeof getTransactionRegister>; onSelect: (voucher: Voucher) => void }) {
  return <Card className="register-print-table overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1040px] text-sm">
    <thead><tr className="bg-muted/50">{['S.No.', 'Date', 'Voucher No.', 'Type', 'Particulars', 'Narration', 'Debit', 'Credit', 'Amount'].map((value, index) => <th key={value} className={cn('report-th', index >= 6 && index <= 8 ? 'text-right' : 'text-left', index === 3 && 'register-print-hide')}>{value}</th>)}<th className="report-th report-controls text-center">Action</th></tr></thead>
    ...
    <tfoot><tr className="register-screen-total border-t-2 bg-muted/30 font-semibold"><td className="report-td" colSpan={6}>Total</td><td className="report-td text-right num"><RegisterMoney value={report.total_debit} /></td><td className="report-td text-right num"><RegisterMoney value={report.total_credit} /></td><td className="report-td text-right num"><RegisterMoney value={report.total_amount} /></td><td className="report-td report-controls" /></tr><tr className="register-print-total hidden border-t-2 bg-muted/30 font-semibold"><td className="report-td" colSpan={5}>Total</td><td className="report-td text-right num">{registerPrintMoney(report.total_debit)}</td><td className="report-td text-right num">{registerPrintMoney(report.total_credit)}</td><td className="report-td text-right num">{registerPrintMoney(report.total_amount)}</td></tr></tfoot>
  </table></div></Card>
}
