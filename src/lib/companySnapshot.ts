...
import { fetchAccounts, fetchAccountCategories, fetchParties, fetchItems, fetchItemCategories, fetchCompanyAccountingSnapshot, fetchPricingRules } from './supabase'
...
export async function fetchCompanySnapshot(company: Company) {
  const [rawAccounts, accountCategories, parties, items, itemCategories, accountingSnapshot, pricingRules] = await Promise.all([
    fetchAccounts(company.id), fetchAccountCategories(company.id), fetchParties(company.id),
    fetchItems(company.id), fetchItemCategories(company.id), fetchCompanyAccountingSnapshot(company.id), fetchPricingRules(company.id),
  ])
  const vouchers = accountingSnapshot.vouchers
