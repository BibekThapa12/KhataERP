...
import {
  ...
  Percent, Boxes, Settings, LogOut, ChevronDown, Code2, CalendarDays, Library, Database, Undo2, Redo2, Menu, X, ListTree, WalletCards, Clock3, Files, Landmark, Plus, CheckCircle2, ArrowLeftRight, Calculator, SlidersHorizontal, HardDrive, ShieldCheck, Activity
} from 'lucide-react'
...
export function AppShell() {
  ...
  }, [developerWorkspace, navigate, readOnly])
    ...
    <div className="app-shell flex h-dvh min-h-0 w-full overflow-hidden bg-background">
      ...
      <aside className={cn('app-shell-sidebar fixed inset-y-0 left-0 z-50 flex w-72 max-w-[85vw] flex-shrink-0 flex-col overflow-y-auto bg-[#1B2A4A] transition-transform md:static md:w-56 md:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <div className="relative px-4 py-5 border-b border-white/10">
          ...
          <div className="text-[10px] uppercase tracking-widest text-blue-200/70 mt-0.5">{developerWorkspace ? 'Developer Workspace' : 'ERP for Nepal'}</div>
          {!developerWorkspace && <CompanySwitcher onSwitched={() => { setMobileOpen(false); navigate('/') }} />}
        </div>
        ...
        <nav className="flex-1 px-2 py-3 space-y-4">
          {developerWorkspace ? (
            <div className="space-y-1">
              <NavLink to="/" onClick={() => setMobileOpen(false)} className="mb-4 flex items-center gap-2.5 rounded-md px-2.5 py-2 text-sm text-blue-100/80 transition-colors hover:bg-white/10 hover:text-white">
                <ArrowLeftRight className="h-4 w-4" />
                <span>Back to ERP</span>
              </NavLink>
              <div className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-blue-300/50">Developer</div>
      ...
      <main className="compact-workspace flex min-w-0 flex-1 flex-col overflow-hidden">
        {!developerWorkspace && <div className="app-shortcuts flex-shrink-0 border-b border-border bg-card px-3 py-2 pl-16 md:px-5" aria-label="Quick shortcuts">
