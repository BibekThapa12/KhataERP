...
begin
  ...
  if position('invoice_item.qty / nullif(coalesce(invoice_item.conversion_factor, 1), 0)' in integrity_definition) = 0
    and position('item.qty / nullif(coalesce(item.conversion_factor, 1), 0)' in integrity_definition) = 0 then
...
-- BEGIN SYNCED MIGRATION: 202609200001_accounting_integrity_manifest.sql
