-- Allow company INSERT triggers to seed protected tenant data before the
-- creator's company_members row exists.
begin;
...
begin
  ...
  -- This must be set before inserting companies: its AFTER INSERT triggers
  -- seed account categories, which are protected by the tenant write guard.
  perform set_config('khataerp.initializing_company', company_id::text, true);
