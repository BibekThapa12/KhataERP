...
const cheque = (overrides: Partial<Cheque> = {}): Cheque => ({
  id:'c1', company_id:'company', direction:'received', cheque_number:'000123', bank_id:'bank', account_number:'00101', party_ledger_id:'party', amount:100,
...
describe('received cheque rules', () => {
  ...
  it('keeps received and issued lists separate', () => {
    const issued=cheque({id:'issued',direction:'issued',bank_id:null,source_account_id:'company-bank'})
    expect(filterPendingCheques([cheque(),issued],'all','2083-03-30','received').map(row=>row.id)).toEqual(['c1'])
    expect(filterPendingCheques([cheque(),issued],'all','2083-03-30','issued').map(row=>row.id)).toEqual(['issued'])
  })
