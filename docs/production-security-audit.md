...
|---|---|---|
| Required environment variables | **Pass** | `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, and the public `VITE_HCAPTCHA_SITE_KEY` are required browser variables. `vite.config.ts` refuses startup/build with missing or placeholder values, rejects a Supabase secret key, requires HTTPS in production, and refuses `VITE_WRITE_PERF=true` in production. Runtime Supabase validation remains in `src/lib/supabase.ts`. |
...
| Authentication rate limiting | **Partial / provider deployment gate** | Login and signup now require a server-verified hCaptcha token and the UI permits at most 5 login attempts/minute and 3 signup attempts/hour per browser session. Browser throttling improves UX but is not an IP security boundary. Supabase Auth provides provider-side rate limits and email recovery throttling, but its password token endpoint defaults do not prove the requested 5/min/IP ceiling. If 5/min/IP is mandatory, put Auth behind a durable edge/WAF rate limiter. No password-reset or OTP UI route exists in this app. |
