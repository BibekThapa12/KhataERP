...
export function CashBankBookPage() {
  ...
  const selectedAccount = accountId === 'all' ? 'All Cash & Bank Accounts' : moneyAccounts.find(account => account.id === accountId)?.name || 'Selected Account'
  const csv = () => downloadCsv('cash-bank-book.csv', ['S.No.', 'Date', 'Voucher', 'Account', 'Activity', 'Receipts', 'Payments', 'Balance'], report.rows.map((row, index) => [index + 1, row.voucher.date_bs, row.voucher.invoice_no || row.voucher.seq, row.account_names, row.activity, row.receipts, row.payments, row.running_balance]))

  return <div className="report-page cash-bank-book-report-page">
    <PageHeader title="Cash & Bank Book" description="Opening, receipts, payments, transfers, and closing balance" action={<div className="flex gap-2"><Button variant="outline" onClick={csv}><Download className="mr-2 h-4 w-4" />CSV</Button><Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button></div>} />
    <PageContent className="report-content space-y-4">
      <FormalReportPrintHeader company={company} title="Cash & Bank Book" periodLabel={`${fmtDate(from)} to ${fmtDate(to)}`} detailLabel={selectedAccount} />
      <Card className="report-controls"><CardContent className="space-y-4 p-4"><div className="max-w-md space-y-1"><Label>Money Account</Label><SearchableSelect value={accountId} onValueChange={setAccountId} options={[{ value: 'all', label: 'All Cash & Bank Accounts' }, ...moneyAccounts.map(account => ({ value: account.id, label: account.name }))]} /></div><div className="flex flex-wrap items-end justify-between gap-4"><ReportDateFilters company={company} range={range} from={from} to={to} onRangeChange={setRange} onFromChange={setFrom} onToChange={setTo} /><label className="flex gap-2 text-sm"><input type="checkbox" checked={showCancelled} onChange={event => setShowCancelled(event.target.checked)} />Show cancelled</label></div></CardContent></Card>
      <div className="report-summary grid gap-3 sm:grid-cols-4">{[['Opening', report.opening], ['Receipts', report.total_receipts], ['Payments', report.total_payments], ['Closing', report.closing]].map(([label, value]) => <Card key={String(label)}><CardContent className="p-4"><p className="text-xs uppercase text-muted-foreground">{label}</p><p className="mt-1 font-serif text-lg font-bold num">Rs {fmtMoney(Number(value))}</p></CardContent></Card>)}</div>
      <Card className="report-table-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="bg-muted/50"><th className="report-th text-left">S.No.</th><th className="report-th text-left">Date</th><th className="report-th text-left">Voucher</th><th className="report-th text-left">Account</th><th className="report-th text-left">Activity</th><th className="report-th text-right">Receipts</th><th className="report-th text-right">Payments</th><th className="report-th text-right">Balance</th></tr></thead><tbody>{report.rows.map((row, index) => <tr key={`${row.voucher.id}-${index}`} className={`border-t ${row.voucher.cancelled ? 'opacity-50 line-through' : ''}`}><td className="report-td text-muted-foreground num">{index + 1}</td><td className="report-td">{fmtDate(row.voucher.date_bs)}</td><td className="report-td">{row.voucher.invoice_no || `${row.voucher.type} ${row.voucher.seq}`}</td><td className="report-td">{row.account_names}</td><td className="report-td">{row.activity}</td><td className="report-td text-right num">{row.receipts ? fmtMoney(row.receipts) : '-'}</td><td className="report-td text-right num">{row.payments ? fmtMoney(row.payments) : '-'}</td><td className="report-td text-right num font-semibold">{fmtMoney(row.running_balance)}</td></tr>)}</tbody><tfoot><tr className="font-semibold"><td className="report-td" colSpan={5}>Closing Balance</td><td className="report-td text-right num">{fmtMoney(report.total_receipts)}</td><td className="report-td text-right num">{fmtMoney(report.total_payments)}</td><td className="report-td text-right num">{fmtMoney(report.closing)}</td></tr></tfoot></table></div></Card>
