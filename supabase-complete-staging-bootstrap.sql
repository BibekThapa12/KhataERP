...
-- Enforce voucher date chronology for automatic voucher numbering.
-- Draft vouchers do not reserve numbers and are ignored. Manual numbering is
-- currently supported only for Journal vouchers and is excluded from chronology.
begin;
...
begin
  ...
  if new.type not in ('Sales', 'Purchase', 'Sales Return', 'Purchase Return', 'Receipt', 'Payment', 'Journal') then
    return new;
  end if;
  ...
  if new.type = 'Journal' and company_record.journal_numbering_mode = 'manual' then
    return new;
  end if;
